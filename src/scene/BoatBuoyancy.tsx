...
const E = 1.5;       // finite-diff step for surface normal
const TILT = 0.45;   // damp tilt so it feels physical
// ponytail: small lift keeps hull above water even if CPU/GPU wave math drifts
const HULL_LIFT = 0.18;

/** Find the first Mesh in a subtree (node may be Group when GLTF shares a mesh). */
...
export function BoatBuoyancy() {
  ...
  const boats = useMemo((): BoatData[] => {
    ...
    // Collect ALL catamaran nodes via traverse (getObjectByName fails for shared-mesh clones)
    ...
    (window as any).__boatNodes = nodes.map(n => ({ name: n.name, isMesh: (n as Mesh).isMesh }));
  ...
  // keep innerRefs sized to match boats
